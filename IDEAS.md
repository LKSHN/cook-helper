...
- One-time data backfill (not a code change): the colors already set via the round swatch were being used as prep-time groupings, not container groupings — so with prepColor freshly split out as its own empty field, "By prep color" sorting had nothing to work with. Copied `color` into `prepColor` for the 53 ingredients currently in the MEP After list (used by a recipe, not MEP-excluded); container color itself was left untouched. The ~40 MEP-excluded ingredients (never shown in After) were left with an empty prep color.

- MEP After list now also has "By prep color", alongside its existing "Alphabetical" / "By container color" — full parity with Before and Shop's sort systems. Defaults to showing/editing container color same as before; only picking "By prep color" switches the swatch to the prep-time one.
